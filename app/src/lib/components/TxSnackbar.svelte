<script lang="ts">
	...
	export let txPromise: Promise<void | FinalExecutionOutcome | FinalExecutionOutcome[]>;
	...
	const outcome: Promise<undefined | TxOutcome[] | { status: 'error'; message: string }> = txPromise
		.then((outcome) => {
	...
	function mapOutcome(outcome: FinalExecutionOutcome): TxOutcome {
		...
		for (const receiptsOutcome of outcome.receipts_outcome) {
			tokensBurnt = tokensBurnt.add(
				new FixedNumber(String(receiptsOutcome.outcome.gas_burnt), TGAS_DECIMALS)
			);
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const receiptStatus = receiptsOutcome.outcome.status as any;
			if (receiptStatus.Failure) {
				status = 'error';
				errorMessage = receiptStatus.Failure.ActionError?.kind?.FunctionCallError?.ExecutionError;
			}
		}
